...
function getCompanyReviewStatus(
...
function CompanyTableLogoCell({
  company,
}: {
  company: Pick<CompanyLeanResponse, "profileImageUrl">;
}) {
...
function CompanyListIdentity({
  ...
  const url = company.profileImageUrl?.trim();
  const imgClass =
    "h-11 w-11 shrink-0 rounded-lg border border-gray-200 bg-gray-50 object-cover";
...
function CompaniesPageContent() {
  ...
  const [filterPendingVerification, setFilterPendingVerification] =
    useState(true);
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      if (
        ...
        !filtersPopoverRef.current.contains(e.target as Node)
      ) {
  ...
  const handleCreateCompany = async (e: React.FormEvent) => {
    ...
    if (
      ...
      !createForm.companyId.trim() ||
      !createForm.email.trim() ||
      !createForm.phoneNumber.trim() ||
      !createForm.physicalAddress.trim()
    ) {
  ...
  const filteredAndSortedCompanies = useMemo(() => {
    ...
    let list = companies.filter((company) => {
      ...
      } else if (reviewTab === "pending" && reviewStatus !== "Pending")
        return false;
      ...
      const noVerificationFilter =
        !filterVerified && !filterPendingVerification;
    ...
    list = [...list].sort((a, b) => {
      ...
      switch (sortBy) {
        case "createdAt":
          cmp =
            toComparableTimestamp(a.createdAt) -
            toComparableTimestamp(b.createdAt);
  ...
  const handleGlobalSortDirectionToggle = () => {
    setSortDir((d) => (d === "asc" ? "desc" : "asc"));
  };

  const handleSortFieldChange = (key: SortKey) => {
    setSortBy(key);
  ...
  const reviewCounts = useMemo(() => {
    ...
    const pending = companies.filter(
      (c) => getCompanyReviewStatus(c) === "Pending",
    ).length;
    const approved = companies.filter(
      (c) => getCompanyReviewStatus(c) === "Approved",
    ).length;
    const rejected = companies.filter(
      (c) => getCompanyReviewStatus(c) === "Rejected",
    ).length;
    return { all, pending, approved, rejected };
  }, [companies]);

  const handleSetCompanyReviewStatus = async (
    company: CompanyLeanResponse,
  ...
  const handleBulkAction = async (action: "approve" | "reject" | "export") => {
    ...
    for (const company of selected) {
      await handleSetCompanyReviewStatus(
        company,
              ...
              <form onSubmit={handleCreateCompany} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCreateForm((f) => ({ ...f, senderId: e.target.value }))
                    }
                    className="w-full border border-gray-300 rounded-lg px-3 py-2"
                  />
                </div>
          ...
          <div className="bg-transparent rounded-xl border border-gray-200/80 shadow-sm overflow-hidden">
            <div className="flex flex-wrap items-center gap-3 px-4 py-3 border-b border-gray-200 bg-gray-50">
              ...
              <div className="relative" ref={filtersPopoverRef}>
                ...
                >
                  ...
                  {(industryFilter ||
                    ...
                    !filterDeactivated) && (
                    <span
                      className="flex h-2 w-2 rounded-full bg-[var(--admin-ui-accent)]"
                      aria-hidden
                    />
                  )}
                </button>
                        ...
                        <div className="space-y-2">
                          ...
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              onChange={(e) =>
                                setFilterPendingVerification(e.target.checked)
                              }
                              className="rounded border-gray-300 text-[var(--admin-ui-accent)] focus:ring-[var(--admin-ui-accent)]"
                            />
                            <span className="text-sm text-gray-700">
                        ...
                        <div className="space-y-2">
                          ...
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              onChange={(e) =>
                                setFilterDeactivated(e.target.checked)
                              }
                              className="rounded border-gray-300 text-[var(--admin-ui-accent)] focus:ring-[var(--admin-ui-accent)]"
                            />
                            <span className="text-sm text-gray-700">
              ...
              <select
                value={sortBy}
                onChange={(e) =>
                  handleSortFieldChange(e.target.value as SortKey)
                }
                className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700"
              >
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
                      <th className="text-left py-3.5 px-4">
                        <input
                          ...
                          checked={
                            ...
                            filteredAndSortedCompanies.every((c) =>
                      ...
                      <th
                        scope="col"
                        className="w-11 py-3.5 pl-3 pr-2 text-left"
                      />
                      <th className="text-left py-3.5 px-5 text-xs font-semibold text-gray-600 uppercase tracking-wider">
                        <button
                          type="button"
                          onClick={handleGlobalSortDirectionToggle}
                          aria-label={`Toggle sort direction. Currently sorted by ${SORT_KEY_LABELS[sortBy]}, ${sortDir === "asc" ? "ascending" : "descending"}.`}
                      ...
                      filteredAndSortedCompanies.map((company) => (
                        ...
                        >
                          ...
                          <td className="py-3.5 px-5 text-sm">
                            ...
                            ) : getCompanyReviewStatus(company) ===
                              "Rejected" ? (
            ...
            >
              {(() => {
                const company = companies.find(
                  (item) => item.id === rowMenu.companyId,
                );
                ...
                return (
                  <>
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        void handleSetCompanyReviewStatus(company, "Approved");
                        setRowMenu(null);
                      }}
                      className="block w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-gray-100"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        void handleSetCompanyReviewStatus(company, "Rejected");
                        setRowMenu(null);
                      }}
                      className="block w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-gray-100"
                    >
