...
function CompanyListIdentity({
  ...
  return (
    <div className="flex min-w-0 items-start gap-3">
      {url ? (
        // eslint-disable-next-line @next/next/no-img-element -- see above
        <img src={url} alt="" loading="lazy" decoding="async" className={imgClass} />
      ) : null}
...
function CompaniesPageContent() {
  ...
  const filteredAndSortedCompanies = useMemo(() => {
    ...
    list = [...list].sort((a, b) => compareCompanies(a, b, sortBy, sortDir))
  ...
  const tableRowVirtualizer = useVirtualizer({
    count: viewMode === 'list' ? filteredAndSortedCompanies.length : 0,
    getScrollElement: () => tableScrollParentRef.current,
    estimateSize: () => 72,
    overscan: 12,
    getItemKey: (index) => filteredAndSortedCompanies[index]?.id ?? index,
  })
      ...
      <div className="flex-1 overflow-auto p-6">
        ...
        {loading ? (
          ...
          <div className="bg-transparent rounded-xl border border-gray-200/80 shadow-sm overflow-hidden">
            ...
            {viewMode === 'list' ? (
            <div ref={tableScrollParentRef} className="max-h-[min(70vh,560px)] overflow-auto">
            <table className="w-full">
              ...
              <tbody>
                {filteredAndSortedCompanies.length === 0 ? (
                  ...
                  <>
                    {(() => {
                      const vItems = tableRowVirtualizer.getVirtualItems()
                      const vTotal = tableRowVirtualizer.getTotalSize()
                      return (
                        <>
                          {vItems.length > 0 ? (
                            <tr className="pointer-events-none" aria-hidden>
                              <td colSpan={8} style={{ height: vItems[0].start, padding: 0, border: 'none' }} />
                            </tr>
                          ) : null}
                          {vItems.map((vRow) => {
                            const company = filteredAndSortedCompanies[vRow.index]
                            return (
                    <tr
                      key={company.id}
                      role="button"
                    ...
                    >
                      ...
                      <td className="relative py-3.5 px-4 text-sm">
                        ...
                        >
                          ...
                          {vItems.length > 0 ? (
                            <tr className="pointer-events-none" aria-hidden>
                              <td colSpan={8} style={{ height: Math.max(0, vTotal - vItems[vItems.length - 1].end), padding: 0, border: 'none' }} />
                            </tr>
                          ) : null}
                        </>
                      )
