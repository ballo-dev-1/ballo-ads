...
function flattenFeatures(
  ...
  return features.flatMap((feature) => [
    feature,
    ...flattenFeatures(feature.subFeatures),
  ]);
}
...
function TreeFeatureNode({
  ...
  const isExpanded = hasChildren
    ? Boolean(expandedFeatures[feature.id])
    : false;
    ...
    <li className="relative pl-5">
      ...
      >
          ...
          <div className="flex items-center gap-2">
            {hasChildren ? (
              <button
                ...
                aria-label={
                  isExpanded
                    ? "Collapse feature branch"
                    : "Expand feature branch"
                }
              >
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px] text-slate-500">
          ...
          <span>
            {formatDateRange(feature.plannedStartDate, feature.plannedEndDate)}
          </span>
        </div>
...
export default function RoadmapPage() {
  ...
  const [overview, setOverview] = useState<RoadmapOverviewResponse | null>(
    null,
  );
  ...
  const [treeDrawerFeature, setTreeDrawerFeature] =
    useState<TreeDrawerFeature>(null);
  ...
  const [drawerStatus, setDrawerStatus] =
    useState<RoadmapFeatureStatus>("Pending");
  ...
  /** Separate horizontal scroller for timeline headers; sticky cannot live inside `overflow-x-auto` on the body. */
  ...
  useEffect(() => {
    ...
    const phase = overview?.phases.find(
      (p) => p.id === treeDrawerFeature.phaseId,
    );
    const feature = phase
      ? findFeatureById(phase.features, treeDrawerFeature.featureId)
      : null;
    if (!feature) return;
    ...
    setDrawerPlannedStartDate(
      feature.plannedStartDate ? feature.plannedStartDate.slice(0, 10) : "",
    );
    setDrawerPlannedEndDate(
      feature.plannedEndDate ? feature.plannedEndDate.slice(0, 10) : "",
    );
  ...
  const summary = useMemo(() => {
    ...
    const completion = overview.totalFeatures
      ? Math.round((overview.doneFeatures / overview.totalFeatures) * 100)
      : 0;
    return {
      ...overview,
      completion,
  ...
  const calendarTimeline = useMemo(() => {
    const yearMonths = calendarRows.flatMap(({ feature }) => {
      ...
      return [start, end].filter(
        (item): item is { year: number; month: number } => item !== null,
      );
    });

    const currentYear = new Date().getFullYear();
    const minYear = yearMonths.length
      ? Math.min(...yearMonths.map((entry) => entry.year))
      : currentYear;
    const maxYear = yearMonths.length
      ? Math.max(...yearMonths.map((entry) => entry.year))
      : currentYear;
    ...
    const columns = years.flatMap((year) =>
      MONTH_LABELS.map((month, monthIndex) => ({ year, month, monthIndex })),
    );
  ...
  const calendarGridMinWidth = Math.max(
    720,
    calendarTimeline.columns.length * CALENDAR_MONTH_WIDTH,
  );
  ...
  const handleCalendarHeaderHorizontalScroll = () => {
    const body = calendarScrollRef.current;
    const head = calendarHeaderScrollRef.current;
    if (!body || !head || body.scrollLeft === head.scrollLeft) return;
  ...
  const handleFeatureStatusChange = async (
    ...
    try {
      ...
      toast.error(
        e instanceof Error ? e.message : "Failed to update feature status",
      );
    }
  };
  ...
  useEffect(() => {
    ...
    const centeredTarget = Math.max(
      0,
      targetPixel - scroller.clientWidth * 0.25,
    );
    const maxScroll = Math.max(0, scroller.scrollWidth - scroller.clientWidth);
    const nextLeft = Math.min(maxScroll, centeredTarget);
    scroller.scrollLeft = nextLeft;
    const headerScroller = calendarHeaderScrollRef.current;
    if (headerScroller) headerScroller.scrollLeft = nextLeft;
    ...
    return (
      <div className="space-y-6 bg-slate-50/70 p-6">
        <AdminHero
          ...
          description="Manage implemented and planned features with a clearer, interactive roadmap board."
        />
  ...
  return (
    <div className="space-y-6 bg-slate-50/70 p-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Switch between hierarchy and calendar timeline
          </p>
        </div>
      ...
      {viewMode === "tree" && overview?.phases.length ? (
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-4 flex items-center justify-between">
            ...
            <p className="text-xs text-slate-500">
              Inspired by product-design roadmap style hierarchy
            </p>
          </div>
      ...
      {viewMode === "calendar" ? (
        <div className="rounded-3xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 px-5 py-4">
            ...
            <p className="text-xs text-slate-500">
              Gantt-style monthly timeline for all planned features
            </p>
          </div>
          <div className="flex min-w-0">
            <div className="w-[260px] shrink-0 border-r border-slate-200 bg-white">
              <div className="sticky top-[-1.5rem] z-20 flex min-h-[4.75rem] flex-col justify-center border-b border-slate-200 bg-slate-50 px-3 py-2">
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              <div className="sticky top-[-1.5rem] z-20 border-b border-slate-200 bg-slate-50 shadow-[0_1px_0_0_rgb(226_232_240)]">
                <div
                  ...
                  onScroll={handleCalendarHeaderHorizontalScroll}
                >
                  <div
                    className="min-w-max"
                    style={{ minWidth: calendarGridMinWidth }}
                  >
                    <div
              ...
              <div
                ...
                onScroll={handleCalendarBodyHorizontalScroll}
              >
                <div
                  className="min-w-max"
                  style={{ minWidth: calendarGridMinWidth }}
                  ...
                  {calendarRows.length ? (
                    calendarRows.map(({ feature }) => {
                      ...
                      const startIndex = start
                        ? (start.year - calendarTimeline.minYear) * 12 +
                          start.month
                        : null;
                      const endIndex = end
                      ...
                      const clampedStart = hasRange
                        ? Math.max(
                            0,
                            Math.min(maxIndex, Math.min(rangeStart, rangeEnd)),
                          )
                        : 0;
                      const clampedEnd = hasRange
                        ? Math.max(
                            0,
                            Math.min(maxIndex, Math.max(rangeStart, rangeEnd)),
                          )
                        : 0;
                          ...
                          >
                            {hasRange ? (
                              <div
                                ...
                                style={{
                                  gridColumn: `${clampedStart + 1} / ${clampedEnd + 2}`,
                                }}
                              ...
                              <div
                                ...
                                style={{
                                  gridColumn: `1 / ${calendarTimeline.columns.length + 1}`,
                                }}
                              >
                                No dates
                              </div>
                    ...
                    <div className="px-4 py-8 text-sm text-slate-500">
                      No features available for calendar view.
                    </div>
                  )}
                </div>
              </div>
      ...
      <RoadmapFormModals
        ...
        featureParentId={
          modal?.mode === "feature" ? modal.parentFeatureId : undefined
        }
        ...
        onSubmitPhase={async (payload) => {
          if (modal?.mode === "phase" && modal.phaseId) {
            ...
            await adminApi.createRoadmapPhase({
              ...payload,
              displayOrder,
              status: "Pending",
            });
        ...
        <div className="flex h-full flex-col">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <select
                  value={drawerStatus}
                  onChange={(e) =>
                    setDrawerStatus(e.target.value as RoadmapFeatureStatus)
                  }
                  className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm"
                >
          ...
          <div className="flex items-center justify-end gap-2 border-t border-slate-200 px-5 py-4">
            ...
            <button
              ...
              disabled={
                savingDrawer || !drawerName.trim() || !drawerUserStory.trim()
              }
